...
INSERT_PREFERENCE = """
    INSERT INTO teacher_preferences (
        teacher_id, day_of_week, time_slot, is_preferred, preference_priority
    ) VALUES (
        %(teacher_id)s, %(day_of_week)s, %(time_slot)s, 
        %(is_preferred)s, %(preference_priority)s
    )
    ON CONFLICT (teacher_id, day_of_week, time_slot)
    DO UPDATE SET 
        is_preferred = EXCLUDED.is_preferred,
        preference_priority = EXCLUDED.preference_priority
"""
...
SELECT_PREFERRED_SLOTS = """
    SELECT day_of_week, time_slot, preference_priority
    FROM teacher_preferences
    WHERE teacher_id = %(teacher_id)s AND is_preferred = true
    ORDER BY preference_priority, day_of_week, time_slot
"""
