...
class Database:
    ...
    def execute_query(
        ...
        Args:
            query: SQL запрос
            params: Параметры запроса (dict для named parameters)
            fetch: Возвращать результаты (True для SELECT)
            commit: Явно указать нужно ли коммитить (None = авто-определение)
            
        Returns:
            Список словарей с результатами или None
        """
        # Auto-detect if commit is needed: commit for INSERT/UPDATE/DELETE
        if commit is None:
            # If query contains RETURNING, it's a modifying query that needs commit
